...
#             # Unlock to get session key
#             unlock_result = subprocess.run(['bw', 'unlock', password, '--raw'], capture_output=True, text=True)
#             if unlock_result.returncode == 0:
#                 self.session_key = unlock_result.stdout.strip()
#                 self.is_logged_in = True
#                 return {"success": True, "message": "Successfully logged in"}
...
#         """Unlock Bitwarden vault"""
#         try:
#             result = subprocess.run(['bw', 'unlock', password, '--raw'], capture_output=True, text=True)
#             if result.returncode == 0:
#                 self.session_key = result.stdout.strip()
#                 self.is_logged_in = True
#                 return {"success": True, "message": "Vault unlocked successfully"}
...
#             result = subprocess.run(['bw', 'list', 'items'], capture_output=True, text=True, env=env)
#             if result.returncode == 0:
#                 items = json.loads(result.stdout)
#                 processed_items = []
#                 for item in items:
...
#                             'name': item.get('name'),
#                             'username': item.get('login', {}).get('username', ''),
#                             'uri': item.get('login', {}).get('uris', [{}])[0].get('uri', '') if item.get('login', {}).get('uris') else '',
#                             'folder': item.get('folderId', ''),
#                             'favorite': item.get('favorite', False),
#                             'organizationId': item.get('organizationId', ''),
#                             'lastModified': item.get('revisionDate', ''),
#                             'strength': self.calculate_password_strength(item.get('login', {}).get('password', ''))
#                         }
#                         processed_items.append(processed_item)

#                 return {"success": True, "message": "Items retrieved successfully", "items": processed_items}
...
#             result = subprocess.run(['bw', 'get', 'password', item_id], capture_output=True, text=True, env=env)
#             if result.returncode == 0:
#                 return {"success": True, "password": result.stdout.strip()}
#             return {"success": False, "message": result.stderr}
#         except Exception as e:
...
#             result = subprocess.run(['bw', 'sync'], capture_output=True, text=True, env=env)
#             if result.returncode == 0:
#                 # Fetch updated items
#                 items_result = bw_manager.get_items()
#                 if items_result["success"]:
...
class BitwardenManager:
    ...
    def login(self, email, password):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                return {"success": False, "message": result.stderr or "Login failed"}
            ...
            return {
                "success": False,
                "message": unlock_result.stderr or "Unlock failed",
            }
        except Exception as e:
            return {
                "success": False,
    ...
    def logout(self):
        ...
        try:
            ...
            if result.returncode == 0:
                return {"success": True, "message": "Successfully logged out"}
            return {"success": False, "message": result.stderr or "Logout failed"}
        except Exception as e:
            return {
                "success": False,
                "message": str(e) or "An unexpected error occurred",
    ...
    def unlock(self, password):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.is_logged_in = True
                return {"success": True, "message": "Vault unlocked successfully"}
            return {"success": False, "message": result.stderr or "Unlock failed"}
        except Exception as e:
            return {
                "success": False,
                "message": str(e) or "An unexpected error occurred",
    ...
    def get_items(self):
        ...
        try:
            ...
            return {
                "success": False,
                "message": result.stderr or "Failed to retrieve items",
            }
        except Exception as e:
            return {
                "success": False,
    ...
    def get_password(self, item_id):
        ...
        try:
            ...
            return {
                "success": False,
                "message": result.stderr or "Failed to retrieve password",
            }
        except Exception as e:
            return {
                "success": False,
...
class GetPassword(Resource):
    def get(self, item_id):
        ...
        return {
            "status": 1 if result["success"] else 0,
            "message": result.get("message", "Failed to retrieve password"),
            "payload": {"password": result.get("password", "")},
        }
    ...
    def post(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                socketio.emit("sync_error", {"message": result.stderr or "Sync failed"})
                return {
                    "status": 0,
                    "message": result.stderr or "Sync failed",
                    "payload": {},
                }
        except Exception as e:
            try:
                ...
                socketio.emit(
                    "sync_error", {"message": str(e) or "An unexpected error occurred"}
                )
            except Exception:
                pass
            return {
