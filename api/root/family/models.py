...
class DeleteNote(Resource):
    ...
    def delete(self, uid, user):
        ...
        note_title = request.args.get("title")  # Add these parameters
        ...
        try:
            # If title and description are provided, delete all matching notes
            if note_title and note_description:
                DBHelper.update_all(  # Use update_all instead of update_one
                    "notes_lists",
                    filters={
                        "title": note_title, 
                ...
                # Fallback to single note deletion
                DBHelper.update_one(
                    "notes_lists",
                    filters={"id": note_id},
                    updates={"is_active": False},
            ...
            return {"status": 1, "message": "Note deleted successfully from all hubs"}
        except Exception as e:
...
class DeleteNoteCategory(Resource):
    ...
    def delete(self, uid=None, user=None):
        try:
            ...
            if not category_id:
                return {"status": 0, "message": "Category ID is required"}, 400
            ...
            # Also soft delete all notes under this category
            DBHelper.update_all(
                table_name="notes_lists",
            ...
            return {"status": 1, "message": "Category and its notes deleted successfully"}
        except Exception as e:
            return {"status": 0, "message": "Failed to delete category", "error": str(e)}, 500
        ...
        for p in projects:
            ...
            if src == "familyhub":
                ...
                if visibility == "private" and source == "planner":
                    filtered_projects.append(p)
                elif visibility == "public" and (source in ["planner", "familyhub"] or source is None):
                    filtered_projects.append(p)
...
class UpdateProject(Resource):
    ...
    def post(self, uid, user):
        ...
        if not project_id:
            return {"status": 0, "message": "id is required"}, 400
        ...
        if not project:
            return {"status": 0, "message": "Project not found or not owned by user"}, 404
...
class UpdateTask(Resource):
    ...
    def post(self, uid, user):
        ...
        task = DBHelper.find_one("tasks", filters={"id": id, "user_id": uid})
        if not task:
