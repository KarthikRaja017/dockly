  ...
  useEffect(() => {
      const fetchUpcomingActivities = async () => {
        try {
          ...
          const normalizeGoals = (res.data?.payload?.goals || [])
          .filter((e: any) => e.provider?.toLowerCase() === "google") 
          .map((g: any) => ({
            summary: g.goal,
            start: {
              dateTime: g.time
                ? new Date(`${g.date} ${g.time}`).toISOString()
          ...
          const normalizeTodos = (res.data?.payload?.todos || []).map((t: any) => ({
            summary: t.text,
            start: {
              dateTime: t.date ? new Date(t.date).toISOString() : null,
              date: t.date,
            },
            provider: "todo",
          }));
