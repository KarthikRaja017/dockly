...
import { TrendingUp, TrendingDown, BarChart3, Activity, ArrowUpRight, ArrowDownRight, Edit } from 'lucide-react';
...
const MarketsWidget: React.FC = () => {
    ...
    const [searchResults, setResults] = useState<{ symbol: string; description: string }[]>([]);
    ...
    if (!markets.length) {
        return <div style={{ padding: 20, textAlign: 'center', color: '#64748b' }}>Loading market data…</div>;
    }

    return (
        <div className="market-card-hover" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: isUpdating ? '#10b981' : '#6b7280', animation: isUpdating ? 'pulse 1s infinite' : 'none' }} />
                    <span style={{ fontSize: 12, color: '#64748b' }}>Live</span>
                    <Edit
                        style={{ color: '#64748b', cursor: 'pointer' }}
                        onClick={() => {
                            setCheckedFavorites(symbols);
                            const merged = Array.from(new Set([...favourites, ...symbols])).slice(0, 15);
                ...
                .map((m) => (
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                            <div>
                                <h4 style={{ fontSize: '14px', fontWeight: '600', color: '#1e293b', margin: '0 0 4px 0' }}>{m.symbol}</h4>
                                <p style={{ fontSize: '20px', fontWeight: '700', color: m.isPositive ? '#10b981' : '#ef4444', margin: '0 0 2px 0' }}>{m.price.toFixed(2)}</p>
                            </div>
                            <div style={{ textAlign: 'right' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'flex-end', marginBottom: '2px' }}>
                                    {m.isPositive ? <ArrowUpRight size={12} style={{ color: '#10b981' }} /> : <ArrowDownRight size={12} style={{ color: '#ef4444' }} />}
                                    <span style={{ fontSize: '12px', color: m.isPositive ? '#10b981' : '#ef4444', fontWeight: '600' }}>
                                        {m.isPositive ? '+' : ''}
                                        {m.change.toFixed(2)}%
                                    </span>
                                </div>
                                <p style={{ fontSize: '10px', color: m.isPositive ? '#10b981' : '#ef4444', fontWeight: '500', margin: 0 }}>
                                    {m.isPositive ? '+' : ''}
            ...
            {/* Remaining Symbols */}
            {markets
                .filter((m) => symbols.includes(m.symbol)) // ✅ only checked symbols
                .slice(1)
                .map((m, i) => (
                    <div key={m.symbol} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: '1px solid #f1f5f9', borderBottom: i === markets.length - 2 ? '1px solid #f1f5f9' : 'none' }}>
                        <div style={{ fontSize: 14, fontWeight: 500 }}>{m.symbol}</div>
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontWeight: 600 }}>{m.price.toFixed(2)}</div>
                            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 4 }}>
                                {m.isPositive ? <TrendingUp size={10} color="#10b981" /> : <TrendingDown size={10} color="#ef4444" />}
                                <span style={{ fontSize: 12, color: m.isPositive ? '#10b981' : '#ef4444' }}>{m.isPositive ? '+' : ''}{m.change.toFixed(2)}%</span>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {/* MODAL */}
            <Modal
                title={<span style={{ fontSize: 18, fontWeight: 600 }}>Customize Market Watchlist</span>}
                open={isModalOpen}
                okText="Save"
                onOk={() => {
                    if (checkedFavorites.length > MAX_MARKETS) {
                        showNotification('Limit reached', `You can track up to ${MAX_MARKETS} markets. Deselect one before saving.`, 'warning');
                        return;
                    }
                    setSymbols(checkedFavorites);
                    localStorage.setItem('selected_symbols', JSON.stringify(checkedFavorites));
                    setModalOpen(false);
                    setShowSearch(false);
                    setSearchTerm('');
                    setResults([]);
                ...
                >
                    ...
                    {showSearch && (
                        <>
                            ...
                            />
                            <div style={{ maxHeight: 150, overflowY: 'auto', paddingRight: 6, marginBottom: 16 }}>
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                if (favourites.length >= 15) {
                                                    showNotification('Limit reached', 'You can only add up to 15 favourites.', 'warning');
                                                    return;
                                                }
                                                const updatedFavs = [...favourites, symbol];
                                                setFavourites(updatedFavs);
                                                localStorage.setItem('favourite_symbols', JSON.stringify(updatedFavs));
                                            }}
                                            style={{
                            ...
                            renderItem={(symbol) => {
                                ...
                                const color = priceAvailable
                                    ? marketData!.isPositive
                                        ...
                                        : '#ef4444'
                                    : '#94a3b8';
                                const priceText = priceAvailable ? marketData!.price.toFixed(2) : '--';
                                ...
                                return (
                                    <List.Item
                                        ...
                                        actions={[
                                            ...
                                            <DeleteOutlined
                                                onClick={() => {
                                                    const updated = favourites.filter((fav) => fav !== symbol);
                                                    setFavourites(updated);
                                                    setCheckedFavorites((prev) => prev.filter((item) => item !== symbol));
                                                    localStorage.setItem('favourite_symbols', JSON.stringify(updated));
                                                }}
                                                ...
                                                <Checkbox
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (checked) {
                                                            ...
                                                            setCheckedFavorites([...checkedFavorites, symbol]);
                                                        } else {
                                                            setCheckedFavorites(checkedFavorites.filter((item) => item !== symbol));
                                                        }
                                                    }}
                                                />
                                            }
                                            title={<span style={{ fontWeight: 500, fontSize: 14 }}>{symbol}</span>}
                                        />
                                    </List.Item>
                                );
                            }}
