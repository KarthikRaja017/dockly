...
const CustomCalendar: React.FC<CalendarProps> = ({
                            ...
                            <div key={index} style={{
                                ...
                                {visibleEvents.map((event, i) => (
                                    <div
                                        ...
                                        onClick={() => handleEventClick(event)}
                                        style={{
                                            backgroundColor: event.type === 'goal' ? '#059669' : 
                                                            event.type === 'todo' ? '#d97706' : 
                                                            event.color || getProviderColor(event.provider || ''), 
                                            color: event.type === 'goal' ? 'white' : 
                                                   event.type === 'todo' ? 'white' : 
                                                   'white',
                                            border: event.type === 'todo' ? '1px solid #d4b106' : 'none',
                                            fontSize: "9px",
                                            padding: "7px 10px",
                                            borderRadius: "4px",
                                            fontWeight: 600,
                                ...
                                {hiddenEventsCount > 0 && (
                                    <div
                                        onClick={() => handleShowAllDayEvents(day)}
                                        style={{
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <div style={{ color: COLORS.textSecondary, fontSize: "12px" }}>
                                    {previewingEvent.provider === 'dockly' ? 'Dockly' : 
                                     previewingEvent.provider === 'google' ? 'Google' : 
                                     previewingEvent.provider === 'outlook' ? 'Outlook' : 
                                     (account?.provider || 'Google')} Account
                                </div>
                            </div>
                        </div>
    ...
    return (
        ...
        `}
            ...
            {/* All Day Events Modal */}
            <AllDayEventsModal
                events={finalData.events.filter(e => {
                    if (!e.is_all_day || !selectedDate) return false;

                    const eventDate = new Date(e.date || e.start_date || e.end_date || "");
                    return (
