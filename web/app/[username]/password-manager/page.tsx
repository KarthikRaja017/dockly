...
const PasswordSetupWizard = () => {
  ...
  const [selectedManagerData, setSelectedManagerData] =
    useState<PasswordManager | null>(null);
  ...
  const passwordManagers = [
    {
      name: "1Password",
      price: "$36/year",
      description: "Best overall option for most users",
      icon: "/api/placeholder/48/48",
    },
    {
      name: "Bitwarden",
      price: "$12/year",
      description: "Great free option with premium features",
      icon: "/api/placeholder/48/48",
    },
    {
      name: "LastPass",
      price: "$36/year",
      description: "Popular choice with wide compatibility",
      icon: "/api/placeholder/48/48",
    },
    {
      name: "Dashlane",
      price: "$40/year",
      description: "Advanced features for security",
      icon: "/api/placeholder/48/48",
    },
    {
      name: "Google Passwords",
      price: "Free",
      description: "Integrated with Google services",
      icon: "/api/placeholder/48/48",
    },
    {
      name: "Apple Keychain",
      price: "Free",
      description: "Built-in for Apple devices",
      icon: "/api/placeholder/48/48",
    },
  ];
  ...
  const renderHeader = () => (
    ...
    >
      <div style={{ fontSize: "22px", fontWeight: 700, color: "#3355ff" }}>
        Dockly
      </div>
          ...
          {steps.map((label, i) => (
            ...
            >
              ...
              <Text
                style={{
                  fontSize: "12px",
                  color: i === currentStep ? "#3355ff" : "#6b7280",
                }}
              >
                {label}
              </Text>
    ...
    <>
      ...
      >
        <Title level={3} style={{ margin: "0", marginRight: "8px" }}>
          Let's set up your password manager
        </Title>
      </div>
      <Text type="secondary" style={{ display: "block", marginBottom: "24px" }}>
        Dockly works best with a password manager for secure, cross-device
      ...
      >
        <LockOutlined
          style={{ fontSize: "20px", color: "#16a34a", marginRight: "12px" }}
        />
        <Text style={{ color: "#16a34a" }}>
          <strong>Security first.</strong> Dockly never stores your actual
          passwords - we work with your password manager to keep your accounts
          secure.
        </Text>
      </div>
      <div style={{ marginBottom: "24px" }}>
        <Text strong style={{ fontSize: "16px" }}>
          Which password manager do you currently use?
        </Text>
      </div>
          ...
          <Col span={8} key={name}>
            <Card
              ...
              style={{
                ...
                borderRadius: "12px",
                border:
                  selectedPasswordManager === name
                    ? "2px solid #8b5cf6"
                    : "1px solid #d1d5db",
              }}
              onClick={() => {
                const selected = passwordManagers.find(
                  (manager) => manager.name === name
                );
                if (selected) {
                  setSelectedPasswordManager(name);
                  setSelectedManagerData(selected);
                } else {
                  setSelectedPasswordManager(null);
  ...
  const renderSetupStep = () => (
    <>
      ...
      <Text type="secondary" style={{ display: "block", marginBottom: "24px" }}>
        A good password manager is essential for your online security and works
      ...
      <div style={{ marginBottom: "32px" }}>
        ...
        <Row gutter={[16, 16]}>
          <Col span={12}>
            <Card style={{ borderRadius: "12px", backgroundColor: "#eff6ff" }}>
              <Text style={{ color: "#2563eb" }}>
                Store unique, strong passwords for all your accounts
              </Text>
            </Card>
          </Col>
          <Col span={12}>
          ...
          <Col span={12}>
            <Card style={{ borderRadius: "12px", backgroundColor: "#eff6ff" }}>
              <Text style={{ color: "#2563eb" }}>
                Protect against data breaches and hacking attempts
              </Text>
            </Card>
          </Col>
          <Col span={12}>
            <Card style={{ borderRadius: "12px", backgroundColor: "#eff6ff" }}>
              <Text style={{ color: "#2563eb" }}>
                Auto-fill logins for quick, easy access
              </Text>
            </Card>
          </Col>
        </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: "32px" }}>
        {passwordManagers
          .slice(0, 3)
          .map(({ name, price, description, icon }) => (
            <Col span={8} key={name}>
              <Card
                hoverable
                style={{
                ...
                onClick={() => {
                  ...
                  if (selected) {
                    ...
                    console.warn(`No password manager found for name: ${name}`);
                  }
                }}
              >
                <div style={{ marginBottom: "12px" }}>
  ...
  const renderIntegrationStep = () => {
    ...
    return (
      <>
        ...
        >
          Your passwords stay private. Dockly never stores your actual
          passwords—we integrate with {selectedManager} for safe and easy
          access.
        </Text>
        <div style={{ marginBottom: "32px" }}>
          <Select
            defaultValue="Browser Extension (Recommended)"
            style={{ width: "100%", marginBottom: "16px" }}
          >
            <Option value="Browser Extension (Recommended)">
              Browser Extension (Recommended)
            </Option>
            <Option value={`${selectedManager} CLI (Advanced)`}>
              {selectedManager} CLI (Advanced)
            </Option>
            <Option value="Manual (No integration)">
              Manual (No integration)
            </Option>
          </Select>
          <div style={{ marginBottom: "16px" }}>
            <Text strong style={{ display: "block", marginBottom: "8px" }}>
              {selectedManager} Account Email
            </Text>
            <Input
          ...
          <Checkbox checked>
            Allow password health monitoring in Dockly
          </Checkbox>
          <Checkbox checked>
            Save new accounts created in {selectedManager}
          </Checkbox>
          <Checkbox checked>
            To complete this integration, we'll need permission from your{" "}
            {selectedManager} browser extension. You'll see a popup requesting
            access when you click "Connect".
          </Checkbox>
        </div>
  ...
  const renderCompleteStep = () => {
    ...
    return (
      <div style={{ textAlign: "center" }}>
        ...
        >
          <CheckCircleOutlined
            style={{ fontSize: "64px", color: "#10b981", marginRight: "16px" }}
          />
          <div>
            <Title level={3} style={{ margin: "0" }}>
              Password manager successfully set up!
            </Title>
          </div>
        </div>
        <Text style={{ display: "block", marginBottom: "24px" }}>
          Your {selectedManager} account is now connected to Dockly. You're
          ready to start managing all your accounts securely from one place.
        </Text>
        <Row
          gutter={16}
        ...
        {selectedManagerData && (
          <div style={{ marginBottom: "32px" }}>
            <Text strong style={{ display: "block", marginBottom: "8px" }}>
              Selected Password Manager Details
            </Text>
            <Text style={{ display: "block" }}>
              Name: {selectedManagerData.name}
            </Text>
            <Text style={{ display: "block" }}>
              Price: {selectedManagerData.price}
            </Text>
            <Text style={{ display: "block" }}>
              Description: {selectedManagerData.description}
            </Text>
          </div>
        )}
    ...
    >
      ...
      >
          ...
          {currentStep > 0 && (
            <>
              {currentStep === 1 && (
                <Button
                  type="link"
                  style={{ color: "#3355ff", marginBottom: "8px" }}
                  onClick={handleBack}
                >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          {currentStep === steps.length - 1 && (
            <Button type="primary" onClick={handleReturnToDashboard}>
              Return to Dashboard
            </Button>
          )}
        </div>
