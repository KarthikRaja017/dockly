...
const Dashboard: React.FC = () => {
  ...
  const handleConnect = async () => {
    if (selectedCalendars.includes('Google Calendar')) {
      const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?` +
        ...
        `redirect_uri=${encodeURIComponent('http://localhost:3000/dashboard')}&` +
        `response_type=code&` +
  ...
  const fetchGoogleCalendarEvents = async (accessToken: string, startDate: Moment, endDate: Moment) => {
    try {
  ...
  const handleViewChange = (mode: 'Day' | 'Week' | 'Month') => {
    ...
    if (accessToken && connectedCalendars.includes('Google Calendar')) {
      const startDate = mode === 'Month' ? moment(selectedDate).startOf('month') : mode === 'Week' ? moment(selectedDate).startOf('week') : moment(selectedDate).startOf('day');
      const endDate = mode === 'Month' ? moment(selectedDate).endOf('month') : mode === 'Week' ? moment(selectedDate).endOf('week') : moment(selectedDate).endOf('day');
    ...
    if (code && selectedCalendars.includes('Google Calendar')) {
      const exchangeToken = async () => {
        try {
          ...
          fetchGoogleCalendarEvents(access_token, moment().startOf('day'), moment().endOf('day'));
          setStep(4);
    ...
    return (
      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {events.map((event, index) => (
          <li key={index} style={{ fontSize: '12px', color: '#2563eb', marginBottom: '4px' }}>
            {event.summary}
          </li>
        ))}
      </ul>
      ...
      <div style={{ display: 'flex', gap: '16px' }}>
              ...
              <div>
                ...
                <div style={{ height: '400px', overflowY: 'auto' }}>
                  {calendarEvents.length > 0 ? (
                    calendarEvents
                      .filter((event) => moment(event.start.dateTime || event.start.date).isSame(selectedDate, 'day'))
                      .map((event, index) => (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                          <div style={{ width: '50px', fontSize: '12px', color: '#6b7280' }}>
                          ...
                          >
                            <p style={{ fontSize: '14px', fontWeight: '500', margin: 0 }}>{event.summary}</p>
                      ...
                      .map((event, index) => (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                          <div style={{ width: '100px', fontSize: '12px', color: '#6b7280' }}>
                            {moment(event.start.dateTime).format('MMM D, h:mm A')}
                          </div>
                          <div
                            style={{
                              flex: 1,
        ...
        <div style={{ flex: 1 }}>
          ...
          <Card style={cardStyle}>
            ...
            <List
              dataSource={[
                { date: '21', title: 'Team Project Review', time: '10:00 AM - 11:30 AM' },
                { date: '23', title: 'Mortgage Payment', time: '$1450.00 - Due' },
                { date: '25', title: 'Family Dinner', time: '6:30 PM - 9:00 PM' },
                { date: '27', title: 'Annual Checkup', time: '9:00 AM - Dr. Johnson' },
              ].concat(
                calendarEvents.map((event) => ({
                  date: moment(event.start.dateTime || event.start.date).format('D'),
                  title: event.summary,
